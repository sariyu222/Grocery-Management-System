...
def customer_register1():
    ...
    if count == 0:
        ...
        return render_template("Mmsg.html", message="Customer Registered Successful")
    else:
...
def add_categories1():
    ...
    try:
        query = {"category_name": category_name, "category_image": category_image.filename}
...
def add_items1():
    ...
    try:
        query = {"item_name": item_name, "item_image": item_image.filename, "item_price": item_price,
...
def get_items():
    ...
    category_col.find(query)
    keyword = request.args.get("keyword")
    keyword = re.compile(".*" + keyword + ".*", re.IGNORECASE)
    query = {"category_name": keyword}
    categories = category_col.find(query)
    categories_ids = []
    for category in categories:
        categories_ids.append({"categories_id": category['_id']})
    categories_ids.append({"item_name": keyword})
    query = {"$or": categories_ids}
    items = items_col.find(query)
    items = list(items)
    return render_template("get_items.html", items=items)#get_category_by_category_id=get_category_by_category_id
...
def update_ordered():
    ...
    customer_order_col.update_one(query,query2)
    return render_template("Amsg.html", message="Order Dispatched")
